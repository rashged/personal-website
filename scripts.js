...
function resetTrack() {
  ...
  for (let i = 4; i < 400; i += 1) {
    ...
    if (i % 15 === 0) {
      const lane = ((Math.sin(i * 2.41) + Math.cos(i * 0.7)) * 0.5 * 0.8);
      state.hazards.push({ z: i, x: lane * (world.width * 0.55), size: 0.8 + Math.random() * 0.4 });
    }
    if (i % 17 === 0) {
      const holeCenter = Math.sin(i * 0.81) * world.width * 0.35;
      state.holes.push({ z: i, x: holeCenter, width: 2.6 + Math.random() * 1.4 });
    }
  }
}
...
function drawHazard(h) {
  ...
  const radius = Math.max(4, h.size * p.scale * 0.32);
...
function checkCollisions() {
  ...
  for (const h of state.hazards) {
    if (Math.abs(h.z - playerZ) < 2.2) {
      const dx = Math.abs(h.x - state.playerX);
      if (dx < state.playerRadius + h.size * 0.45) return true;
    }
  }

  for (const hole of state.holes) {
    if (Math.abs(hole.z - playerZ) < 2) {
      if (Math.abs(hole.x - state.playerX) < hole.width * 0.5 - 0.2) return true;
    }
  }
...
function tick(now) {
  ...
  if (state.running) {
    ...
    speedEl.textContent = `${(state.speed / 15).toFixed(1)}x`;
  }
